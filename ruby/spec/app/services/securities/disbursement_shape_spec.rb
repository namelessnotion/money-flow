...
RSpec.describe Services::Securities::DisbursementShape do
  ...
  let(:cash_id) { Services::Securities::Leg.cash_id_for(payout_id) }
  ...
  context 'when the repayment returns principal as well as interest' do
    ...
    it "pays the same amount from the Security's cash into the Investor's cash, so they can withdraw it" do
      leg = request.transfers[cash_id]

      expect(leg.from_wallet_id).to eq(world.security_wallet('security_cash'))
      expect(leg.to_wallet_id).to eq(world.wallet_of(world.investor, 'cash'))
      expect(leg.amount.minor_units).to eq(43_000)
    end
    ...
    it 'stages nothing and mints nothing' do
      [payout_id, cash_id, retirement_id].each do |id|
  ...
  context 'when the repayment is interest only' do
    ...
    it 'pays the interest on both legs' do
      ...
      expect(request.transfers[cash_id].amount.minor_units).to eq(2_500)
    end
  end
